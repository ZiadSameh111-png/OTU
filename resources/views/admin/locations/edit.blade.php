        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">تعديل موقع: {{ $location->name }}</h3>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('admin.locations.update', $location) }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.0000001" name="latitude" id="latitude" class="form-control @error('latitude') is-invalid @enderror" value="{{ old('latitude', $location->latitude) }}" required>
                                    @error('latitude')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.0000001" name="longitude" id="longitude" class="form-control @error('longitude') is-invalid @enderror" value="{{ old('longitude', $location->longitude) }}" required>
                                    @error('longitude')
                                        <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="range_meters" id="range_meters" class="form-control @error('range_meters') is-invalid @enderror" value="{{ old('range_meters', $location->range_meters) }}" required min="10">
                            @error('range_meters')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // استخدام إحداثيات الموقع الحالي
        const locationLat = {{ $location->latitude }};
        const locationLng = {{ $location->longitude }};
        const locationRadius = {{ $location->range_meters }};
        
        // تهيئة الخريطة
        map = L.map('map').setView([locationLat, locationLng], 15);
        ...
        marker = L.marker([locationLat, locationLng], {
            draggable: true
        }).addTo(map);
        
        // رسم دائرة تمثل نطاق الموقع
        circle = L.circle([locationLat, locationLng], {
        ...
        // تحديث حقول النموذج عند تحريك العلامة
        ...
        // تحديث نصف قطر الدائرة عند تغيير حقل النطاق
        document.getElementById('range_meters').addEventListener('change', function() {
            updateCircle();
        });
